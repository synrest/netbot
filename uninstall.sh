...
if command -v systemctl >/dev/null 2>&1; then systemctl --user disable --now netbot-watch.service >/dev/null 2>&1 || true; fi
rm -f "$BIN/netbot" "$BIN/netbot-watch"
rm -f "$HOME/.config/systemd/user/netbot-watch.service"
