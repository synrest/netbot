...
from .presentation import (dashboard as dashboard_view, status as status_view,
                            ...
                            render_maintain, render_doctor, render_accept, render_reject, render_merge)
...
Use 'netbot <command> --help' for details.""")
    ...
    if a.verbose and a.json:
        p.error("--json and --verbose cannot be used together")
    if a.command is None:
        data = dashboard_view(a.config, a.db)
        print(json.dumps(data, indent=2, sort_keys=True) if a.json else render_dashboard(data), end="")
        return
    if a.command == "enroll":
        if len(raw_argv) != 1:
    ...
    if a.command == "cycle":
        if a.host or a.target or a.candidate or a.target_filter:
            p.error("usage: netbot cycle [--dry-run]")
        print(json.dumps(run_cycle(a.config, a.db, dry_run=a.dry_run), indent=2, sort_keys=True))
        return
    if a.command == "maintain":
        if a.host or a.target or a.candidate or a.target_filter:
            p.error("usage: netbot maintain [--dry-run]")
        result = run_maintenance(a.config, a.db, dry_run=a.dry_run)
        print(json.dumps(result, indent=2, sort_keys=True) if a.json else render_maintain(result, verbose=a.verbose, config=a.config, db=a.db), end="")
        return
    if a.command == "scheduler":
        ...
        return
    if a.command == "events":
        if a.host or a.target or a.candidate:
            p.error("usage: netbot events")
        payload = events_view(a.config, a.db)
        print(json.dumps(payload, indent=2, sort_keys=True) if a.json else render_events(payload), end="")
        return
    if a.command == "accept":
        if not a.host or a.target or a.candidate or a.target_filter or a.proposal_type or a.proposal_node:
            p.error("usage: netbot accept NODE [--dry-run] [--json]")
    ...
    if a.command == "reject":
        ...
        return
    if a.command == "merge":
        if not a.host or not a.merge_survivor or a.target or a.candidate or a.target_filter:
            p.error("usage: netbot merge SOURCE --into SURVIVOR [--dry-run] [--json]")
        result = merge_topology(a.config, a.db, a.host, a.merge_survivor, dry_run=a.dry_run)
        print(json.dumps(result, indent=2, sort_keys=True) if a.json else render_merge(result), end="")
        return
    ...
    if a.command == "doctor":
        ...
        payload = diagnose()
        print(json.dumps(payload, indent=2, sort_keys=True) if a.json else render_doctor(payload), end="")
        return
    if a.command == "sync":
    ...
    if a.command == "ssh":
        ...
        if a.host == "apply-target":
            ...
            return
        if a.host == "adopt-target":
            if not a.target or not a.expected_sha256:
                p.error("usage: netbot ssh adopt-target TARGET --expected-sha256 SHA256 [--dry-run]")
            plan = managed_adoption_plan(a.config, a.target, a.expected_sha256, db_path=a.db)
            result = plan if a.dry_run else apply_managed_adoption(plan, a.config, db_path=a.db)
            print(json.dumps(result, indent=2, sort_keys=True))
            return
        if a.host == "activate-target":
            ...
            plan = build_activation_plan(a.config, a.target, db_path=a.db,
                                         authorize_existing_config=getattr(a, "authorize_existing_config", False))
    ...
    if a.command == "inspect" and not a.probe:
        payload = inspect_view(a.config, a.db, a.host or "")
        print(json.dumps(payload, indent=2, sort_keys=True) if a.json else render_inspect(payload), end="")
        return
    if a.command == "status":
        payload = status_view(a.config, a.db)
        print(json.dumps(payload, indent=2, sort_keys=True) if a.json else render_status(payload), end="")
        return
