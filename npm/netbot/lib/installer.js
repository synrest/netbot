...
function validatePython(python) {
  const result = cp.spawnFileSync(python, ['-c', 'import sys,venv,ensurepip; print(sys.version_info[:2])'], {encoding: 'utf8'});
...
function install(options = {}) {
  ...
  try {
    ...
    const extracted = cp.spawnFileSync(python, [extract, artifact, staging, `netbot-${version}`], {encoding: 'utf8'});
    ...
    const made = cp.spawnFileSync(python, ['-m', 'venv', venv], {encoding: 'utf8'});
    if (made.error || made.status !== 0) throw new Error('private Python runtime creation failed');
    const site = cp.spawnFileSync(path.join(venv, 'bin', 'python'), ['-c', 'import site; print(site.getsitepackages()[0])'], {encoding: 'utf8'});
    ...
    const checked = cp.spawnFileSync(path.join(venv, 'bin', 'python'), ['-c', 'import netbot.cli,netbot.version; print(netbot.version.__version__)'], {encoding: 'utf8'});
    ...
    try {
      ...
      const launchCheck = cp.spawnFileSync(path.join(finalDir, 'venv', 'bin', 'python'), ['-m', 'netbot.cli', '--version'], {encoding: 'utf8', env: {...process.env, NETBOT_PREFIX: root}, cwd: root});
